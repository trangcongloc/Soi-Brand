 ...
 *
 * Tests the VEO video analysis feature with 4 concurrent browser tabs
 * processing different YouTube videos simultaneously.
 *
 * Run with: npx playwright test tests/e2e/veo-simultaneous.spec.ts --headed
 */
...
test.describe('VEO Simultaneous Processing Test', () => {
  ...
  test('4 tabs can process different videos simultaneously', async ({ browser }) => {
    console.log('\n========================================');
    console.log('Starting Simultaneous VEO Test');
    console.log('========================================\n');
    ...
    try {
      // Step 1: Navigate all tabs to VEO page
      console.log('Step 1: Navigating all tabs to VEO page...');
      await Promise.all(pages.map(async (page, index) => {
        await page.goto(VEO_URL);
        await waitForPageLoad(page);
        console.log(`  Tab ${index + 1}: Loaded VEO page`);
      }));

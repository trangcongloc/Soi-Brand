...
import { VeoUrlInput } from "./PromptUrlInput";
import { VeoScriptInput } from "./PromptScriptInput";
...
function VeoForm({ onSubmit, onError, isLoading, hasApiKey = true, geminiModel, keyStatus = "unverified" }: VeoFormProps) {
  ...
  const [workflow, setWorkflow] = useState<PromptWorkflow>("url-to-scenes");
  ...
  const savedSettings = useMemo(() => loadPromptFormSettings(), []);

  // Settings — initialized from persisted values
  const [mode, setMode] = useState<PromptMode>(savedSettings.mode);
  ...
  const handleFileUpload = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (!file.name.endsWith(".txt") && !file.name.endsWith(".json")) {
        onError(lang.prompt.form.errors.invalidFileType);
        return;
      }
      ...
      reader.onerror = () => {
        onError(lang.prompt.form.errors.cannotReadFile);
      };
  ...
  const handleSubmit = useCallback(
    (e: React.FormEvent) => {
      ...
      if (workflow === "url-to-script" || workflow === "url-to-scenes") {
        if (!url.trim()) {
          onError(lang.prompt.form.errors.emptyUrl);
          return;
        }
        if (!isValidYouTubeUrl(url)) {
          onError(lang.prompt.form.errors.invalidUrl);
          return;
        }
        onSubmit({
          workflow,
        ...
        if (!scriptText.trim()) {
          onError(lang.prompt.form.errors.emptyScript);
          return;
        }
        onSubmit({
    ...
    <div className={styles.container}>
      ...
      {!hasApiKey && (
        <div className={styles.apiKeyNotice}>
          ...
          <span>{lang.prompt.form.noApiKey}</span>
        </div>
      )}
      ...
      {hasApiKey && keyStatus === "verifying" && (
        <div className={styles.keyVerifying}>
          ...
          <span>{lang.prompt.form.verifyingKey || "Verifying API key..."}</span>
        </div>
      )}
      ...
      {hasApiKey && keyStatus === "invalid" && (
        <div className={styles.keyInvalid}>
          ...
          <span>{lang.prompt.form.invalidKey || "Invalid API key"}</span>
        </div>
      )}
      ...
      {hasApiKey && keyStatus === "valid" && geminiModel && (
        <div className={styles.keyValid}>
          ...
          <span>{lang.prompt.form.usingModel}: {geminiModel}</span>
        </div>
      )}
        ...
        {showSceneSettings && (
          <AnimatePresence>
            {hasInput && (
              ...
              >
                {isLoading
                  ? lang.prompt.form.processing
                  : workflow === "url-to-scenes"
                  ? lang.prompt.form.generateFromUrl
                  : lang.prompt.form.generateScenes}
              </motion.button>
            )}
          </AnimatePresence>
        )}

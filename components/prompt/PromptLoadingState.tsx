    ...
    <div className={styles.container}>
      ...
      >
        ...
        <div className={styles.header}>
          ...
          <h2 className={styles.title}>{lang.prompt.loading.title}</h2>
        </div>
        ...
        <div className={styles.progressContainer}>
          ...
          <div className={styles.progressInfo}>
            {showBatchProgress && (
              <span>
                {lang.prompt.loading.batchProgress
                  .replace("{current}", String(batch))
                  .replace("{total}", String(totalBatches))}
              </span>
            )}
            <span>
              {lang.prompt.loading.scenesGenerated.replace(
                "{count}",
              ...
              <div className={styles.scriptPreviewHeader}>
                ...
                <div className={styles.scriptPreviewActions}>
                  <button
                    ...
                    type="button"
                  >
                    {showScriptPreview ? lang.prompt.loading.hideScript : lang.prompt.loading.showScript}
                  </button>
                  <button
                    ...
                    type="button"
                  >
                    {copied ? lang.prompt.result.sceneCard.copied : lang.prompt.script.copy}
                  </button>
                  <button
                    className={styles.scriptActionBtn}
              ...
              <div className={styles.scriptMeta}>
                {generatedScript.title && (
                  <span className={styles.scriptMetaItem}>
                    <strong>{lang.prompt.script.videoTitle}:</strong> {generatedScript.title}
                  </span>
                )}
                {generatedScript.duration && (
                  <span className={styles.scriptMetaItem}>
                    <strong>{lang.prompt.script.duration}:</strong> {generatedScript.duration}
                  </span>
                )}
                {generatedScript.characters?.length > 0 && (
                  <span className={styles.scriptMetaItem}>
                    <strong>{lang.prompt.script.characters}:</strong> {generatedScript.characters.join(", ")}
                  </span>
                )}
              </div>
              <AnimatePresence>

...
import { Scene, CharacterRegistry, PromptJobSummary, GeneratedScript } from "@/lib/prompt";
import { downloadFile as downloadFileUtil } from "@/lib/prompt/download-utils";
import styles from "./PromptDownloadPanel.module.css";
...
function VeoDownloadPanel({
  ...
  const downloadOptions = [
    {
      label: lang.prompt.result.downloadOptions.scenesJson,
      description: lang.prompt.result.downloadOptions.scenesJsonDesc.replace("{count}", String(scenes.length)),
      icon: (
        <svg
          width="20"
          height="20"
    ...
    {
      label: lang.prompt.result.downloadOptions.scenesTxt,
      description: lang.prompt.result.downloadOptions.scenesTxtDesc.replace("{count}", String(scenes.length)),
      icon: (
        <svg
          width="20"
          height="20"
    ...
    ...(script ? [{
      label: lang.prompt.result.downloadOptions.script,
      description: lang.prompt.result.downloadOptions.scriptDesc,
      icon: (
        <svg
          width="20"
          height="20"
    ...
    {
      label: lang.prompt.result.downloadOptions.characters,
      description: lang.prompt.result.downloadOptions.charactersDesc.replace("{count}", String(Object.keys(characterRegistry).length)),
      icon: (
        <svg
          width="20"
          height="20"
  ...
  return (
    <div className={styles.container}>
      <h3 className={styles.title}>{lang.prompt.result.downloadOptions.title}</h3>

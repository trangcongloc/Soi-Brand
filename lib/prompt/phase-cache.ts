...
const PHASE_CACHE_PREFIX = "prompt_phase_";
...
const phaseCache = new LocalStorageCache<PromptPhaseCache>({
 ...
 */
export function getPhaseCache(jobId: string): PromptPhaseCache | null {
...
function ensurePhaseCache(
  jobId: string,
  videoUrl: string,
  settings: PromptPhaseCache["settings"]
): PromptPhaseCache {
...
export function cachePhase0(
  ...
  const updated: PromptPhaseCache = {
    ...cache,
    timestamp: Date.now(),
 ...
 */
export function getAllPhaseCaches(): PromptPhaseCache[] {
  if (!isBrowser()) return [];

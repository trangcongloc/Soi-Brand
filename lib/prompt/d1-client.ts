/**
 * Prompt Pipeline - Cloudflare D1 Client (Server-side only)
 * REST API client for D1 database operations
 */

import { gzipSync, gunzipSync } from "zlib";
import { CachedPromptJob, CachedPromptJobInfo, PromptJobStatus } from "./types";
 ...
 */
export async function listJobs(): Promise<CachedPromptJobInfo[]> {
  const now = Date.now();
  ...
  const rows = await queryD1<{
    ...
    `SELECT * FROM prompt_jobs WHERE expires_at > ? ORDER BY updated_at DESC`,
    [now]
  );
 ...
 */
export async function getJob(jobId: string): Promise<CachedPromptJob | null> {
  const now = Date.now();

  const rows = await queryD1<{ data_compressed: string; expires_at: number }>(
    `SELECT prompt_job_data.data_compressed, prompt_jobs.expires_at
     FROM prompt_job_data
     JOIN prompt_jobs ON prompt_job_data.job_id = prompt_jobs.job_id
     WHERE prompt_job_data.job_id = ? AND prompt_jobs.expires_at > ?`,
    [jobId, now]
  );
  ...
  const job: CachedPromptJob = JSON.parse(decompressed.toString("utf-8"));
 ...
 */
export async function upsertJob(jobId: string, job: CachedPromptJob): Promise<void> {
  ...
  await queryD1(
    `INSERT OR REPLACE INTO prompt_jobs (
  ...
  await queryD1(
    `INSERT OR REPLACE INTO prompt_job_data (job_id, data_compressed) VALUES (?, ?)`,
    [jobId, base64]
  );
...
export async function deleteJob(jobId: string): Promise<void> {
  await queryD1(`DELETE FROM prompt_jobs WHERE job_id = ?`, [jobId]);
  // Cascade deletes prompt_job_data automatically
}
...
export async function deleteAllJobs(): Promise<void> {
  await queryD1(`DELETE FROM prompt_jobs`);
  await queryD1(`DELETE FROM prompt_job_data`);
}
...
export async function cleanupExpired(): Promise<void> {
  const now = Date.now();
  await queryD1(`DELETE FROM prompt_jobs WHERE expires_at < ?`, [now]);
}

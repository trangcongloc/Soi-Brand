 ...
 * mood associations, and usage contexts for VEO 3 prompting.
 */
...
/**
 * Generic color filter utility (internal)
 */
type ColorFilterKey = "usageContexts" | "moods" | "temperature";

function filterColors<K extends ColorFilterKey>(
  key: K,
  value: K extends "temperature"
    ? ColorTemperature
    : K extends "moods"
    ? ColorMood
    : ColorUsage
  ...
  if (key === "temperature") {
    return CINEMATIC_COLORS.filter(
      (color) => color.temperature === value
    );
  } else if (key === "usageContexts") {
    return CINEMATIC_COLORS.filter((color) =>
      (color.usageContexts as ColorUsage[]).includes(value as ColorUsage)
    );
  } else if (key === "moods") {
    return CINEMATIC_COLORS.filter((color) =>
      (color.moods as ColorMood[]).includes(value as ColorMood)
    );
  }
  return [];
}
...
/**
 * Get all colors for a specific usage context
 */
export function getColorsByUsage(
  usage: ColorUsage
): CinematicColorDefinition[] {
  return filterColors("usageContexts", usage);
}

/**
 * Get all colors with a specific mood
 */
export function getColorsByMood(mood: ColorMood): CinematicColorDefinition[] {
  return filterColors("moods", mood);
}

/**
 * Get all colors of a specific temperature
 */
export function getColorsByTemperature(
  temperature: ColorTemperature
): CinematicColorDefinition[] {
  return filterColors("temperature", temperature);
}

/**
 * Get a specific color by ID
 */
export function getColorById(id: string): CinematicColorDefinition | undefined {
  return CINEMATIC_COLORS.find(color => color.id === id);
}

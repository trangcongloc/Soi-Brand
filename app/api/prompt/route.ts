    ...
    async start(controller) {
      ...
      try {
        ...
        if (promptRequest.workflow === "url-to-script") {
          const script = await runUrlToScript(promptRequest, apiKey, sendEvent);
          ...
          sendEvent({
            ...
            data: {
              ...
              summary: {
                ...
                youtubeUrl: promptRequest.videoUrl!,
                videoId,
                targetScenes: 0,
                actualScenes: 0,
        ...
        // Handle URL to Scenes (combined workflow)
          ...
          try {
            ...
            if (promptRequest.useVideoTitle && videoInfo?.title) {
              videoTitle = videoInfo.title;
            }
            // Collect full description text (if enabled)
            if (promptRequest.useVideoDescription && videoInfo?.description) {
              videoDescriptionText = videoInfo.description;
            }
            // Parse video description for chapters (if enabled)
            if (promptRequest.useVideoChapters && videoInfo?.description) {
          ...
          let effectiveSceneCount = promptRequest.sceneCount;
          let extractedColorProfile: CinematicProfile | undefined = promptRequest.existingColorProfile as CinematicProfile | undefined;

          // DIRECT MODE: Video → Scenes (no script generation)
          if (promptRequest.mode === "direct") {
            // Calculate scene count from YouTube duration
            if (promptRequest.sceneCountMode !== "manual" && youtubeDurationSeconds > 0) {
            ...
            // ============================================================================
            if (promptRequest.extractColorProfile === true && !extractedColorProfile) {
              try {
                sendEvent({
                  event: "progress",
                ...
                const colorResult = await extractColorProfileFromVideo(promptRequest.videoUrl!, {
                  apiKey,
                  model: promptRequest.geminiModel,
                  onProgress: (msg) => {
                    sendEvent({
                      event: "progress",
          ...
          // HYBRID MODE: Video → Script → Scenes
            ...
            script = await runUrlToScript(promptRequest, apiKey, sendEvent, videoDescription, videoTitle, videoDescriptionText, promptRequest.useVideoCaptions);
            ...
            if (promptRequest.sceneCountMode !== "manual") {
            ...
            try {
              ...
              // BUG FIX #9: Add timeout wrapper around Phase 1 to prevent indefinite hangs
              const characterDataPromise = extractCharactersFromVideo(promptRequest.videoUrl!, {
                apiKey,
                model: promptRequest.geminiModel,
                onProgress: (msg) => {
                  sendEvent({
              ...
              if (pendingHybridPhase1LogId) {
                sendEvent({
                  ...
                  data: {
                    ...
                    error: { type: "TIMEOUT", message: error.message },
                    request: {
                      model: promptRequest.geminiModel || "gemini-2.0-flash-exp",
                      body: "",
                      promptLength: 0,
                      videoUrl: promptRequest.videoUrl,
                    },
                    response: {
                      success: false,
                      body: "",
          ...
          const batchCount = promptRequest.mode === "direct"
            ? Math.ceil(youtubeDurationSeconds / (promptRequest.batchSize * DEFAULT_SECONDS_PER_SCENE))
            : Math.ceil(effectiveSceneCount / promptRequest.batchSize);

          // Build summary
          const summary: PromptJobSummary = {
            mode: promptRequest.mode as PromptMode,
            youtubeUrl: promptRequest.videoUrl!,
            videoId,
            targetScenes: effectiveSceneCount,
            actualScenes: result.scenes.length,
            batches: batchCount,
            batchSize: promptRequest.batchSize,
            voice: promptRequest.voice === "no-voice" ? "No voice (silent)" : promptRequest.voice,
        ...
        // Handle Script to Scenes workflow
          ...
          if (promptRequest.mode === "direct") {
            result = await runScriptToScenesDirect(promptRequest, apiKey, sendEvent);
          } else {
          ...
          const summary: PromptJobSummary = {
            ...
            youtubeUrl: "",
            videoId: "",
            targetScenes: promptRequest.sceneCount,
            actualScenes: result.scenes.length,
            batches:
              promptRequest.mode === "hybrid"
                ? Math.ceil(promptRequest.sceneCount / promptRequest.batchSize)
                : 1,
            batchSize: promptRequest.batchSize,
            voice: promptRequest.voice === "no-voice" ? "No voice (silent)" : promptRequest.voice,
            charactersFound: Object.keys(result.characterRegistry).length,

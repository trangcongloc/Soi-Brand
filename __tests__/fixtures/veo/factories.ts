...
import { CACHE_TTL_MS } from "@/lib/veo/constants";
...
export function createMockJobSummary(options: CreateMockJobSummaryOptions = {}): VeoJobSummary {
...
export interface CreateMockJobOptions {
  ...
  summaryOverrides?: Partial<VeoJobSummary>;
}

export function createMockJob(options: CreateMockJobOptions = {}): CachedVeoJob {
...
export function createMockProgress(options: CreateMockProgressOptions = {}): VeoProgress {
  const sceneCount = options.sceneCount ?? 10;
...
export function createMockResumeData(options: CreateMockResumeDataOptions = {}): VeoResumeData {
  const sceneCount = options.sceneCount ?? 10;
  const batchSize = options.batchSize ?? 5;

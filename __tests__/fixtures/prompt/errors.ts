...
export const ERROR_TYPES: Record<string, PromptErrorType> = {
...
/**
 * Create a PromptErrorEvent from an error scenario
 */
export function createErrorEvent(scenario: ErrorScenario): PromptErrorEvent {
  return {
    event: "error",
    data: {
      type: scenario.type,
 ...
 */
export function isRetryableErrorType(type: PromptErrorType): boolean {
 ...
 */
export function getErrorMessage(type: PromptErrorType): string {
  const messages: Record<PromptErrorType, string> = {
